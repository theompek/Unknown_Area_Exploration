...
class Cffi:
    ...
    def brushfireFromObstacles(ogm, brush, ogml):
        ...
        numpy_implementation = rospy.get_param("numpy_implementation")
    ...
    def thinning(skeleton, ogml):
        ...
        if numpy_implementation == True:
            ...
            # TODO: Must be faster!
            itime = time.time()
            skeleton[:skeleton.shape[0], :skeleton.shape[1]] = np.array(y)
...
class OgmOperations:
    ...
    def blurUnoccupiedOgm(ogm, ogml):
        ...
        if numpy_implementation == True:
            local = np.copy(ogm)
            temp = np.ones((ogml['max_x'] - ogml['min_x'] + 3, ogml['max_y'] - ogml['min_y'] + 3))
            temp[np.where(ogm[ogml['min_x'] - 1: ogml['max_x'] + 2, ogml['min_y'] - 1: ogml['max_y'] + 2] > 49)] = 0
            nonvMat = np.ones((3, 3))
            cMat = signal.convolve2d(temp, nonvMat, 'valid')
            mask = np.where((cMat >= 4) & (temp[1:-1, 1:-1] == 0))
